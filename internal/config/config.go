	...
	cfg := &Config{
		...
		Models: map[string]ModelConfig{
			"test-model": {
				...
				Capabilities:  []string{"chat", "completions", "embeddings", "images", "moderations"},
			},
		},
	}
	cfg.applyDefaults()
...
func (c *Config) Validate() error {
	...
	for externalModel, model := range c.Models {
		...
		for _, capability := range model.Capabilities {
			switch capability {
			case "chat", "completions", "embeddings", "images", "moderations", "transcriptions", "translations", "speech", "batches", "files", "fine_tuning":
			default:
