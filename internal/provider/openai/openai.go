...
func (p *Provider) UploadFile(ctx context.Context, req compat.FileUploadRequest) (*compat.FileObject, error) {
	...
	httpReq, err := http.NewRequestWithContext(ctx, http.MethodPost, p.endpoint("/files"), bytes.NewReader(body))
	if err != nil {
		return nil, err
	}
	p.setHeaders(httpReq)
...
func (p *Provider) ListFiles(ctx context.Context) (*compat.FileList, error) {
	httpReq, err := http.NewRequestWithContext(ctx, http.MethodGet, p.endpoint("/files"), nil)
	if err != nil {
		return nil, err
	}
	p.setHeaders(httpReq)
...
func (p *Provider) RetrieveFile(ctx context.Context, fileID string) (*compat.FileObject, error) {
	httpReq, err := http.NewRequestWithContext(ctx, http.MethodGet, p.endpoint("/files/"+fileID), nil)
	if err != nil {
		return nil, err
	}
	p.setHeaders(httpReq)
...
func (p *Provider) DownloadFile(ctx context.Context, fileID string) ([]byte, string, error) {
	httpReq, err := http.NewRequestWithContext(ctx, http.MethodGet, p.endpoint("/files/"+fileID+"/content"), nil)
	if err != nil {
		return nil, "", err
	}
	p.setHeaders(httpReq)
