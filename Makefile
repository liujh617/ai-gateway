...
.PHONY: fmt check-line-endings test-line-endings test race vet verify build run check-config check-config-examples smoke smoke-responses smoke-responses-tools smoke-responses-state smoke-rate-limit smoke-azure smoke-deepseek smoke-deepseek-skip release-check docker-build docker-run
...
smoke-responses-state:
	bash scripts/smoke-responses-state.sh
