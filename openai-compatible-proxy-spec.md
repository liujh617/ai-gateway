...
OpenAI-compatible provider 会尽量保留 upstream error 中的 `message`、`type`、`param` 和 `code`。上游错误响应体超过网关读取上限，或不是单个 JSON 值时，错误体会被视为不可信并回退到默认错误映射。上游 `5xx` 会映射为网关 `502`，避免把上游内部状态直接暴露为网关自身故障。
