...
网关可以配置单个 `api_key`、多个 `api_keys`，或带非敏感名称的 `api_clients`。当 `api_clients` 非空时，任一 client 的 token 都可以通过鉴权，并使用 client `name` 作为日志、metrics 和限流标签；client 可通过 `models` 配置可见模型白名单，未配置或为空时可访问全部模型。`api_key` 和 `api_keys` 继续保留用于兼容早期配置，默认 client 标签分别为 `default` 和 `key_1`、`key_2` 等。`api_key` 和各类 gateway key 不允许为空或包含首尾空白。`GATEWAY_API_KEY` 可以覆盖单个 key，`GATEWAY_API_KEYS` 可以用逗号分隔的多个 key 覆盖列表；空片段会被视为配置错误。配置自检、日志和 metrics 不得输出实际 gateway API key。
...
返回网关对当前调用方可见的模型列表。若 gateway client 配置了 `models` 白名单，该列表只包含白名单内模型。

`HEAD /v1/models` 需要 Bearer token，返回相同状态码和响应头，但不返回 body。

## `GET /v1/models/{model}`

返回网关静态配置中的单个模型元数据。该路由需要 Bearer token，并执行与模型列表及推理请求相同的 gateway client 模型白名单检查。

已知且对当前 client 可见的模型返回与 `GET /v1/models` 列表元素相同的对象。`HEAD /v1/models/{model}` 返回相同状态码和响应头，但不返回 body。

模型不存在或不在当前 client 的模型白名单内时，统一返回 `404 invalid_request_error`，避免泄露隐藏模型是否存在。该查询只读取本地静态路由配置，不调用上游 provider，也不检查 provider health。
...
网关支持文本和 function tools 的 Responses API 子集。请求必须包含 `model` 和 `input`；`input` 可以是非空字符串，或由 `user`、`assistant`、`system`、`developer` 文本消息组成的非空数组。message `content` 可以是非空字符串，或只包含 `{ "type": "input_text", "text": "..." }` 的非空数组。可选 `instructions` 会转换为首条 `developer` message。`stream` 默认 `false`，`store` 默认 `true`。

网关把请求转换为现有 chat completions provider 调用，因此模型必须具备 `chat` capability；provider adapter 不需要实现 Responses 专属接口。非流式结果返回 `object: "response"`，并在 `output` 中返回一个 completed assistant message Item，其 content 使用 `output_text` part。provider usage 映射为 `input_tokens`、`output_tokens` 和 `total_tokens`。

流式响应使用 typed SSE，每个事件同时包含 `event:` 和 `data:`。正常文本流依次发送 `response.created`、`response.in_progress`、`response.output_item.added`、`response.content_part.added`、一个或多个 `response.output_text.delta`、`response.output_text.done`、`response.content_part.done`、`response.output_item.done` 和 `response.completed`。Responses stream 不发送 `[DONE]`。流开始后的错误发送 `error` typed event 并关闭连接；客户端取消会关闭上游 stream。
...
该受保护端点读取当前 gateway client 先前成功保存的完整 Response。GET 返回创建时的 Response JSON，包括原始 response ID、output item ID、usage、tools、`previous_response_id` 和 `store`；HEAD 返回相同状态和 `Content-Type: application/json`，但不返回 body。读取只访问本地 response store，不调用 provider，也不检查 provider health。

只有采用默认 `store: true` 或显式 `store: true` 且完整成功的非流式响应，以及正常发送 `response.completed` 的流式响应可以读取。未知 ID、`store: false`、store 禁用、过期、容量淘汰和属于其他 gateway client 的记录统一返回 `404 invalid_request_error`，`param` 为 `response_id`。具体 ID 路径在日志和 metrics 中规范化为 `/v1/responses/{response_id}`。

读取复用现有进程内 TTL、LRU、条目数和 byte 限制；byte 限制同时计算 continuation transcript 和完整 Response JSON。状态在进程重启后丢失且不跨实例共享。当前不支持 cancel、input items、`include` 查询选项或持久化外部存储。

### `DELETE /v1/responses/{response_id}`

该受保护端点删除当前 gateway client 的目标记录。成功返回 `200` 和 `{ "id": "resp_...", "object": "response", "deleted": true }`。删除后目标无法再通过 GET 读取或作为 `previous_response_id` 使用；已经基于它创建的后续 Response 是独立快照，不受影响。
...
`tools` 支持 `type: "function"`、`name`、可选 `description`、object `parameters` 和布尔 `strict`。省略 `strict` 时按 Responses 语义补为 `true`。支持 `tool_choice` 的 `auto`、`none`、`required` 和指定 function，并支持 `parallel_tool_calls`。built-in、custom、namespace、allowed tools、tool search 和 MCP tools 返回 `400 invalid_request_error`。

无状态第二轮调用必须在同一 `input` 中按顺序带回 completed `function_call` 和匹配 `call_id` 的字符串 `function_call_output`。使用 `previous_response_id` 时可以只发送 `function_call_output`，但 call ID 必须存在于已存储的历史中。网关将其转换为 Chat Completions assistant `tool_calls` 和 tool message。

非流式结果把上游 tool calls 转为独立 completed `function_call` Items。流式结果为每个调用发送 `response.output_item.added`、`response.function_call_arguments.delta`、`response.function_call_arguments.done` 和 `response.output_item.done`；多个调用可交错，最终在 `response.completed.output` 中返回完整 Items。
...
Token 指标仅统计上游响应中明确返回的 `usage`，不做估算。当前覆盖非流式 chat completions、embeddings，以及流式 chat completions 中带 `usage` 的 SSE chunk；流式响应如未返回最终 usage，不记录 token 指标。
...
Rate limit rejection 指标在 gateway 自身 rate limiter 拒绝请求并返回 `429 rate_limit_error` 时递增。该指标不统计上游 provider 返回的 `429`。

Rate limit rejection 指标标签：

- `path`
- `client`

Provider circuit open 指标在 provider 因进程内 circuit breaker 处于 unhealthy 而被跳过时递增。

Provider circuit open 指标标签：
...
Provider health 指标标签：

- `provider`
- `state`: `healthy` 或 `unhealthy`
...
```json
{
  "models": {
    "priced-chat-model": {
      ...
      "pricing": {
        "prompt_usd_per_1m_tokens": 1.00,
        "completion_usd_per_1m_tokens": 2.00
      },
      "fallbacks": [
        {
          "provider": "backup-openai",
          "upstream_model": "backup-chat-model",
          "pricing": {
            "prompt_usd_per_1m_tokens": 1.50,
            "completion_usd_per_1m_tokens": 2.50
          }
        }
      ]
    }
...
整个配置段缺省时使用以上默认值。任一字段为 `0` 时 store 整体禁用，负值使配置校验失败。容量满时先清理过期记录，再按 LRU 淘汰。store 可能包含 prompt、completion 和工具参数等敏感数据，只保存在内存中。
...
`rate_limit.requests_per_minute` 是全局默认值。`api_clients[].rate_limit.requests_per_minute` 可覆盖单个 gateway client；未配置时继承全局默认值，显式配置为 `0` 表示该 client 关闭限流。
...
- `0` 表示关闭限流。
- 限流 key 优先使用鉴权后的 gateway client name。
- 旧配置未设置 client name 时，单个 `api_key` 的 client name 为 `default`，`api_keys` 的 client name 为 `key_1`、`key_2` 等。
- 公开路由 `/healthz`、`/readyz`、`/version` 和 `/metrics` 不参与限流。
- 超限返回 `429 rate_limit_error`，响应包含 `Retry-After`，取值为当前固定窗口剩余秒数且至少为 `1`，并递增 `open_ai_gateway_rate_limit_rejections_total`。
...
- `error_type`
- `error_code`
- `previous_response`（仅布尔值，不记录 response ID）

日志不得记录 `Authorization` header、上游 API key、完整 prompt 或完整 completion。

Responses continuation 的完整 `previous_response_id` 只允许进入显式启用的 audit event；普通 access log 和 metrics 均不得记录该 ID。

`client` 字段使用非敏感 gateway client name；公开路由为 `public`，鉴权失败为 `unauthenticated`，未配置 gateway API key 时为 `unconfigured`。
...
网关支持显式启用的本地 JSONL 审计模式，用于研究自己使用的第三方 agent 如何组织上下文、提示词、工具调用参数和响应。该模式不是默认 access log，默认关闭。
...
- `GATEWAY_AUDIT_ENABLED`: 支持 `1`、`true`、`yes`、`on`、`0`、`false`、`no`、`off`。
- `GATEWAY_AUDIT_PATH`: 覆盖 JSONL 文件路径。
- `GATEWAY_AUDIT_MAX_FILE_BYTES`: 覆盖单个审计文件大小上限；`0` 表示不轮转。

当 `audit.max_file_bytes` 大于 `0` 时，写入下一条事件前如果当前文件会超过阈值，网关会将当前文件轮转为 `<path>.1`，再创建新的当前文件。单条事件本身超过阈值时仍会写入，不丢弃事件。

客户端可以传入 `X-Agent-Trace-Id` 关联一次 agent session；未传入时使用网关 request id 作为 `trace_id`。
...
- 审计文件会记录完整请求和响应，可能包含 prompt、completion、tool schema、embedding input 和 embedding vector。
...
自检会加载配置、应用默认值、执行配置校验，并以 snake_case JSON 字段输出 listen addr、gateway client、server runtime、log、rate limit、provider health、provider/model 摘要和 warning。gateway client 摘要只包含非敏感 `name`、可见模型白名单和限流覆盖；provider 摘要包含 `timeout_seconds` 和 API key 是否配置等非敏感状态；model 和 fallback 摘要会显示默认补齐后的有效 `upstream_model`；输出不得包含 API key 明文。
