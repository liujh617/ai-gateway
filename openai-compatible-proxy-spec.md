...
网关支持无状态、纯文本的 Responses API 子集。请求必须包含 `model` 和 `input`；`input` 可以是非空字符串，或由 `user`、`assistant`、`system`、`developer` 文本消息组成的非空数组。message `content` 可以是非空字符串，或只包含 `{ "type": "input_text", "text": "..." }` 的非空数组。可选 `instructions` 会转换为首条 `developer` message。`stream` 默认 `false`，`store` 只能省略或显式为 `false`。
...
当前不支持 `previous_response_id`、`store: true`、Conversations、非 function 工具、reasoning Items、图片、音频、文件、多模态、background mode、WebSocket 和其他未知字段。这些字段不会被静默忽略，而是稳定返回 `400 invalid_request_error`。
...
无状态第二轮调用必须在同一 `input` 中按顺序带回 completed `function_call` 和匹配 `call_id` 的字符串 `function_call_output`。call ID 必须唯一，output 必须匹配更早的 call；网关将其转换为 Chat Completions assistant `tool_calls` 和 tool message。
