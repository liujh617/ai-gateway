...
读取复用现有进程内 TTL、LRU、条目数和 byte 限制；byte 限制同时计算 continuation transcript 和完整 Response JSON。状态在进程重启后丢失且不跨实例共享。当前不支持删除 Response、cancel、input items、`include` 查询选项或持久化外部存储。
