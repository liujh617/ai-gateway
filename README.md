...
- `response_store`: `/v1/responses` 会话状态和完整 Response JSON 的进程内存储；默认 TTL 3600 秒、1000 条、单条 4 MiB、总计 64 MiB，任一限制为 `0` 时整体禁用。
...
- 有 TTL 和容量上限的单进程 Responses store，同时保存 continuation transcript 和完整 Response JSON；重启后状态丢失
