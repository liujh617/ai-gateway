...
自检输出会用 snake_case JSON 字段显示 listen addr、gateway client、server runtime、log、rate limit、provider health、provider/model 摘要和 warning；provider 摘要包含 `timeout_seconds` 和 API key 是否配置等非敏感状态，不会输出 API key 明文。
